...
def sd15_ella_hidiffusion_key():
    modified_key = dict()
    modified_key['down_module_key'] = ['unet.down_blocks.0.downsamplers.0.conv']
    modified_key['down_module_key_extra'] = ['unet.down_blocks.1']
    modified_key['up_module_key'] = ['unet.up_blocks.2.upsamplers.0.conv']
    modified_key['up_module_key_extra'] = ['unet.up_blocks.2']
    modified_key['windown_attn_module_key'] = ['unet.down_blocks.0.attentions.0.transformer_blocks.0', 
...
def apply_hidiffusion(
    ...
    diffusion_model.info = {'size': None, 'hooks': [], 'scheduler': model.scheduler, 'use_controlnet': hasattr(model, 'controlnet'),
                            'is_inpainting_task': False, 'is_playground': False}
    ...
    if name_or_path in ['runwayml/stable-diffusion-v1-5', 'stabilityai/stable-diffusion-2-1-base']:
        ...
        for key, module in diffusion_model.named_modules():          
            if 'ella' in key:
                print(key)
                modified_key = sd15_ella_hidiffusion_key()
                break
        else:
